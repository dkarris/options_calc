...
# function to clean datestamp: take date from micex and add 18:45 minutes
# to make a proper datetime
...
b_asset = pd.DataFrame.from_records(
    r.json()['securities']['data'], columns=r.json()['securities']['columns'], index='SECID')

# clean datetine columns

b_asset['LASTTRADEDATE'], b_asset['LASTDELDATE'] = pd.to_datetime(
    b_asset['LASTTRADEDATE']), pd.to_datetime(b_asset['LASTDELDATE'])
...
nearest_ticket = b_filtered['LASTTRADEDATE'].apply(
    lambda g: (g.date() - today).days).idxmin()
...
b = pd.DataFrame.from_records(
    r.json()['marketdata']['data'], columns=r.json()['marketdata']['columns'], index='SECID')
...
b_opt['LASTTRADEDATE'], b_opt['LASTDELDATE'] = pd.to_datetime(
    b_opt['LASTTRADEDATE']), pd.to_datetime(b_opt['LASTDELDATE'])
...
b_opt['_OPT_TYPE'] = ['Call' if element[element.find(
    "B", 2) + 1:element.find("B", 2) + 2] in call_definition else 'Put' for element in index_values]
...
b_opt_filtered[
    b_opt_filtered['_OPT_TYPE'] == 'Call'].sort_values(by=['LASTTRADEDATE'])
